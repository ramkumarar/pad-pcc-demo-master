...
public class CustomerController {
	...
	@ResponseBody
	public String directCacheAccess(@RequestParam(value = "email", required = true) String email) throws Exception {
		System.out.println(clientCache.rootRegions().size());
		Region customerRegion = clientCache.getRegion("customer");
		Customer customer  = (Customer)customerRegion.get(email);
		customer.setName(customer.getName().concat("ram"));
		customerRegion.put(email, customer);
		// customer =(Customer)customerRegion.get(email);
		return "hello";
